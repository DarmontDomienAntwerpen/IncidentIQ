...
def load_video(url, lang="Nederlands"):
    msgs = {
        ...
        "English":    {"cached":"✓ Already loaded — using Pinecone cache. Ready!","loaded":"✓ Video loaded and ready for questions!"},
        "Français":   {"cached":"✓ Déjà chargée — cache Pinecone. Prête!","loaded":"✓ Vidéo chargée et prête!"},
    }
...
# ── PDF ────────────────────────────────────────────────────────────────────────
def make_pdf(language="dutch", source_url=""):
    RED=HexColor("#C0392B"); DARK=HexColor("#1C2833"); ORANGE=HexColor("#E67E22"); GREEN=HexColor("#1E8449"); WHITE=white
    lang = {"dutch":"Dutch","english":"English","french":"French"}.get(language,"Dutch")
    results = vs.similarity_search("key points lessons learned recommendations conclusions important facts", k=12)
    if not results: return None, "No video content found."
    ...
    raw = re.sub(r'```json|```','',llm.invoke(
        ...
        f'Rules: max 15 words per item, no timestamps.\n\nContext:\n{context}\n\nJSON:'
    ).content.strip()).strip()
    data = json.loads(raw)
    ...
    c.drawString(3.1*cm,y-0.32*cm,"Automatically extracted from video — not cited from a person.")
    y-=1.1*cm
...
def build_agent():
    ...
    def search_video_knowledge(query: str) -> str:
        """Search the Pinecone knowledge base for information relevant to the query. Use this tool to answer questions about the loaded video content. Uses query rewriting and multi-query for maximum retrieval quality. Automatically translates non-English queries. Returns relevant transcript excerpts with timestamp sources."""
        try:
            ...
            try: qs = json.loads(re.sub(r'```json|```','',llm.invoke(f"Generate 3 search query variations. Return JSON list.\nQuestion: {rw}\nJSON:").content.strip()).strip())
            except: qs = [rw]
            qs.append(rw)
            all_docs = {}
            for q in qs:
    ...
    def generate_xvr_scenario(language: str = "dutch") -> str:
        """Generate a structured XVR simulation scenario brief based on the loaded incident video. Use when user asks to create an XVR scenario. Specify language as dutch, english or french."""
        try:
            results = vs.similarity_search("location building fire cause complications decisions resources weather time casualties evacuation", k=12)
            ...
            return llm.invoke(
                ...
                f"DEBRIEFING:\n1. [Q]\n2. [Q]\n3. [Q]\n\n"
                f"XVR NOTES:\n[Key moments]\n\nRules: base on context only.\n\nContext:\n{context}\n\nBrief:"
            ).content.strip()
        except Exception as e: return f"Error: {e}"

    @tool
    def generate_visual_summary(language: str = "dutch") -> str:
        """Generate structured JSON for visual timeline. Use when user asks for visual summary or timeline. Returns valid JSON."""
        try:
            results = vs.similarity_search("timeline events cause complications lessons learned mistakes decisions outcome", k=12)
            if not results: return "No video content found."
            context = "\n\n".join([re.sub(r"\[\d{2}:\d{2}\]\s*(?=\[\d{2}:\d{2}\])","",r.page_content) for r in results])
            lang = {"dutch":"Dutch","english":"English","french":"French"}.get(language.lower(),"Dutch")
            raw = re.sub(r'```json|```','',llm.invoke(
                f'Extract structured info in {lang}. Return ONLY valid JSON:\n'
                f'{{"title":"Short title","subtitle":"Presenter","duration":"duration",'
                ...
                f'"timeline":[{{"timestamp":"00:00","title":"Event","text":"Max 2 sentences.","quote":"","tags":["tag"],"color":"blue","badge":"Context"}}],'
                f'"learnings":[{{"number":"01","title":"Title","text":"Max 2 sentences."}}],'
                f'"source_url":""}}\n\n'
                f'Rules: exactly 4 metrics with REAL values, 4-6 events, 4 learnings, all text in {lang}.\n\nContext:\n{context}\n\nJSON:'
            ).content.strip()).strip()
            json.loads(raw)
            return raw
        except Exception as e: return f"Error: {e}"
    ...
    def send_gmail(pdf_path: str = "", text_content: str = "", subject_suffix: str = "Document", custom_emails: str = "") -> str:
        """Send generated document via Gmail. pdf_path: PDF to attach. text_content: email body. subject_suffix: label. custom_emails: recipients."""
        try:
            ...
            msg = MIMEMultipart(); msg["From"]="me"; msg["To"]=", ".join(recipients); msg["Subject"]=subject
    ...
    TOOLS = [search_video_knowledge, generate_xvr_scenario, generate_visual_summary, send_gmail]
...
Tools: search_video_knowledge, generate_xvr_scenario, generate_visual_summary, send_gmail
ROUTING: Question->search | XVR->generate_xvr_scenario | Visual->generate_visual_summary | Email->send_gmail
CRITICAL: Always respond in EXACT same language as user. Dutch->Dutch. English->English. French->French.
...
def ask(message, language="Nederlands"):
    ...
    for tc in calls: add_trace(tc,f"lang:{LANG_TOOL.get(language,'dutch')}",lat=lat/max(len(calls),1),tokens=tok//max(len(calls),1),cost=cost/max(len(calls),1))
    return final,calls,lat

def render_trace(pro):
    steps=S["trace"][-10:]
    if not steps: return "<div style='color:#555;font-size:11px;text-align:center;padding:12px'>Nog geen activiteit...</div>"
    html=""
    if pro and S["run_id"]:
        html+=f"<div style='font-size:9px;color:#555;font-family:monospace;padding:6px 8px;background:#161616;border-radius:6px;margin-bottom:8px;border:0.5px solid #2a2a2a;line-height:1.7'>RUN {S['run_id']} · {S['thread_id'][:16]}<br>model: gpt-4o-mini · embed: text-embedding-3-small · index: incidentiq</div>"
    for i,step in enumerate(steps):
        is_last=i==len(steps)-1
        if pro:
            lat_s=f" · {step['lat']:.2f}s" if step.get("lat") else ""
            tok_s=f" · {step['tokens']} tok" if step.get("tokens") else ""
            connector = '<div style="width:1px;flex:1;background:#2a2a2a;margin-top:3px;min-height:10px"></div>' if not is_last else ''
            html+=f"<div style='display:flex;gap:6px;margin-bottom:5px'><div style='display:flex;flex-direction:column;align-items:center;width:10px;flex-shrink:0;padding-top:3px'><div style='width:8px;height:8px;border-radius:50%;background:#1D9E75;flex-shrink:0'></div>{connector}</div><div style='flex:1;background:#161616;border-radius:6px;padding:6px 8px;border:0.5px solid #2a2a2a;border-left:2px solid #1D9E75'><div style='font-size:10px;font-family:monospace;color:#aaa'><span style='background:#0d1f14;color:#1D9E75;padding:1px 5px;border-radius:3px;font-size:9px'>ok</span> <span style='color:#C0392B'>tool /</span> {step['label']}<span style='color:#555'>{lat_s}{tok_s}</span></div><div style='font-size:9px;color:#555;font-family:monospace;margin-top:2px'>{step.get('detail','')}</div></div></div>"
        else:
            icons={"fetch_youtube_transcript":"Video laden","search_video_knowledge":"Zoeken in de video","generate_xvr_scenario":"XVR scenario maken","generate_visual_summary":"Tijdlijn genereren","send_gmail":"Versturen naar team"}
            label=icons.get(step["label"],step["label"])
            lat=f" · {step['lat']:.1f}s" if step.get("lat") else ""
            html+=f"<div style='display:flex;align-items:center;gap:8px;padding:7px 10px;border-radius:6px;margin-bottom:4px;background:#0d1f14;border:0.5px solid #1D9E7533'><span style='color:#1D9E75;font-size:13px'>✓</span><span style='font-size:12px;color:#ddd;flex:1'>{label}</span><span style='font-size:10px;color:#555;font-family:monospace'>{lat}</span></div>"
    if pro and S["total_tokens"]:
        ...
        html+=f"<div style='margin-top:8px;padding:6px 8px;background:#161616;border-radius:5px;border:0.5px solid #2a2a2a;font-size:9px;font-family:monospace;color:#555;line-height:1.8'>{'─'*34}<br>{len(steps)} tool calls · {S['total_tokens']:,} tokens · ${S['total_cost']:.6f}{ls}</div>"
    return html

def render_timeline(json_str):
    try: data=json.loads(json_str)
    except: return "<div style='color:#aaa;padding:20px'>Geen tijdlijn data.</div>"
    CM={"red":("#C0392B","#1a0d0d","#ff6b6b"),"amber":("#E67E22","#1a1200","#ffa94d"),"green":("#1D9E75","#0d1a14","#69db7c"),"blue":("#4a9eff","#0d1220","#74c0fc")}
    html=f"<div style='background:linear-gradient(135deg,#1C2833,#2C3E50);padding:20px 24px;border-radius:12px;margin-bottom:20px'><div style='font-size:9px;letter-spacing:0.12em;color:#C0392B;background:#C0392B22;padding:3px 10px;border-radius:4px;border:1px solid #C0392B44;display:inline-block;margin-bottom:10px;font-weight:500'>INCIDENT ANALYSIS</div><div style='font-size:19px;font-weight:500;color:white;margin-bottom:4px'>{data.get('title','')}</div><div style='font-size:12px;color:rgba(255,255,255,0.5)'>{data.get('subtitle','')} · {data.get('duration','')}</div></div>"
    metrics=data.get("metrics",[])
    if metrics:
        html+="<div style='display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:20px'>"
        for m in metrics:
            ch,bh,th=CM.get(m.get("color","blue"),CM["blue"])
            html+=f"<div style='background:#1a1a1a;border-radius:10px;padding:14px;border-bottom:3px solid {ch}'><div style='font-size:24px;font-weight:500;color:white;font-family:monospace;line-height:1'>{m.get('value','')}<span style='font-size:12px;color:#555;font-weight:400'> {m.get('unit','')}</span></div><div style='font-size:11px;color:#777;margin-top:5px'>{m.get('label','')}</div></div>"
        html+="</div>"
    ...
    for ev in data.get("timeline",[]):
        ...
        qh=f"<div style='border-left:2px solid {ch};padding-left:10px;margin:8px 0;font-size:12px;color:#aaa;font-style:italic;line-height:1.6'>{ev['quote']}</div>" if ev.get("quote") else ""
        ...
        html+=f"<div style='display:flex;gap:0;margin-bottom:2px'><div style='width:52px;flex-shrink:0;padding-top:14px;text-align:right;padding-right:10px'><span style='font-size:10px;color:#555;font-family:monospace'>{ev.get('timestamp','')}</span></div><div style='width:20px;flex-shrink:0;display:flex;flex-direction:column;align-items:center;padding-top:14px'><div style='width:10px;height:10px;border-radius:50%;background:{ch};box-shadow:0 0 0 3px {ch}33;flex-shrink:0'></div><div style='width:1px;flex:1;background:#2a2a2a;min-height:20px'></div></div><div style='flex:1;padding:8px 0 16px 12px'><div style='background:#1a1a1a;border-radius:10px;padding:14px 16px;border:0.5px solid #2a2a2a;border-left:3px solid {ch}'><div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;gap:8px'><div style='font-size:13px;font-weight:500;color:white'>{ev.get('title','')}</div><span style='font-size:10px;padding:2px 8px;border-radius:4px;background:{bh};color:{th};font-weight:500;white-space:nowrap;flex-shrink:0'>{ev.get('badge','')}</span></div><div style='font-size:12px;color:#aaa;line-height:1.7'>{ev.get('text','')}</div>{qh}<div style='margin-top:8px'>{tgs}</div></div></div></div>"
    html+="<div style='font-size:10px;letter-spacing:0.09em;color:#555;font-weight:500;margin:16px 0 10px'>KEY LEARNINGS</div><div style='display:grid;grid-template-columns:1fr 1fr;gap:8px'>"
    for l in data.get("learnings",[]): html+=f"<div style='background:#1a1a1a;border-radius:10px;padding:14px;border:0.5px solid #2a2a2a'><div style='font-size:10px;color:#C0392B;font-family:monospace;font-weight:500;margin-bottom:6px'>{l.get('number','')}</div><div style='font-size:12px;font-weight:500;color:white;margin-bottom:5px'>{l.get('title','')}</div><div style='font-size:11px;color:#777;line-height:1.6'>{l.get('text','')}</div></div>"
    html+="</div>"
    return html

def video_status_html():
    if S["video_loaded"]:
        return f"<div style='background:#0d1f14;border-radius:8px;padding:10px 12px;border:0.5px solid #1D9E7533;margin:4px 0'><div style='font-size:10px;color:#1D9E75;font-weight:500;margin-bottom:2px'>● Video geladen</div><div style='font-size:11px;color:#aaa'>{S['video_title'][:40]}</div></div>"
    return "<div style='background:#1a1a1a;border-radius:8px;padding:10px 12px;border:0.5px solid #2a2a2a;margin:4px 0;font-size:11px;color:#555'>Geen video geladen</div>"
...
def handle_pdf(language):
    if not S["video_loaded"]: return "⚠️ Laad eerst een video.", None, render_trace(False)
    lt = LANG_TOOL.get(language,"dutch")
    try:
        fp,data = make_pdf(lt, S.get("video_url",""))
        S["pdf_path"]=fp
        add_trace("generate_pdf_cheatsheet",f"lang:{lt}",lat=2.1,tokens=500,cost=0.000075)
        return f"✓ **PDF aangemaakt:** {data.get('title','')}", fp, render_trace(False)
    except Exception as e: return f"❌ Error: {e}", None, render_trace(False)

def handle_timeline(language):
    if not S["video_loaded"]: return "<div style='color:#aaa;padding:20px'>⚠️ Laad eerst een video.</div>", render_trace(False)
    lt=LANG_TOOL.get(language,"dutch")
    result,_,lat = ask(f"Generate a visual timeline summary in {lt}", language)
    try:
        json.loads(result); S["visual_json"]=result
        return render_timeline(result), render_trace(False)
    except: return f"<div style='color:#aaa;padding:20px'>❌ Error: {result[:200]}</div>", render_trace(False)

def handle_xvr(language):
    if not S["video_loaded"]: return "⚠️ Laad eerst een video.", None, render_trace(False)
    lt=LANG_TOOL.get(language,"dutch")
    result,_,lat = ask(f"Generate an XVR scenario brief in {lt}", language)
...
def handle_send(email_to, doc_choice, language):
    if not email_to.strip(): return "⚠️ Vul een e-mailadres in.", render_trace(False)
    lt=LANG_TOOL.get(language,"dutch")
    if "PDF" in doc_choice and S["pdf_path"]: result,_,_=ask(f"Send the PDF at {S['pdf_path']} to {email_to} with subject_suffix 'Key Concepts Cheatsheet'",language)
    elif "XVR" in doc_choice and S["xvr_content"]: result,_,_=ask(f"Send this XVR scenario to {email_to} with subject_suffix 'XVR Scenario Brief':\n{S['xvr_content'][:500]}",language)
    else: result,_,_=ask(f"Send a visual summary to {email_to} with subject_suffix 'Visual Summary'",language)
    add_trace("send_gmail",f"to:{email_to}",lat=0.5)
    return result or "✓ Verstuurd!", render_trace(False)
...
with gr.Blocks(title="IncidentIQ") as demo:
    ...
    <div style='display:flex;align-items:center;justify-content:space-between;
        ...
        <div style='display:flex;gap:8px'>
            <span style='font-size:10px;color:#C0392B;background:#C0392B11;padding:3px 10px;border-radius:20px;border:0.5px solid #C0392B44'>gpt-4o-mini</span>
            <span style='font-size:10px;color:#4a9eff;background:#4a9eff11;padding:3px 10px;border-radius:20px;border:0.5px solid #4a9eff44'>Pinecone</span>
            <span style='font-size:10px;color:#1D9E75;background:#1D9E7511;padding:3px 10px;border-radius:20px;border:0.5px solid #1D9E7544'>LangSmith</span>
        </div>
    </div>""")
    ...
    with gr.Row():
        with gr.Column(scale=1, min_width=240):
            ...
            gr.HTML("<div style='font-size:10px;letter-spacing:0.08em;color:#444;font-weight:500;margin:10px 0 6px'>GENEREREN</div>")
            btn_pdf = gr.Button("📄  Key Concepts PDF", variant="secondary", size="sm")
            btn_tl  = gr.Button("📊  Visuele tijdlijn",  variant="secondary", size="sm")
            btn_xvr = gr.Button("🎮  XVR Scenario",      variant="secondary", size="sm")
            ...
            doc_choice  = gr.Dropdown(["📄 Key Concepts PDF","📊 Visuele tijdlijn","🎮 XVR Scenario"], value="📄 Key Concepts PDF", label="Document")
            btn_send    = gr.Button("📤  Versturen", variant="primary", size="sm")
            ...
            if os.getenv("LANGSMITH_API_KEY"):
                gr.HTML("<a href='https://smith.langchain.com' target='_blank' style='display:block;padding:7px;border-radius:7px;border:0.5px solid #2a2a2a;font-size:11px;color:#C0392B;text-decoration:none;text-align:center;margin-top:8px'>📊 LangSmith traces →</a>")

        with gr.Column(scale=3):
            with gr.Tabs():
                with gr.Tab("💬  Chat"):
                    chatbot   = gr.Chatbot(value=[], height=420, label="Chat", type="messages")
                    with gr.Row():
                        msg_input = gr.Textbox(placeholder="Stel een vraag of drop een YouTube URL...", label="Input", lines=1, scale=10)
                        send_chat = gr.Button("→", scale=1, variant="primary", size="sm")
                with gr.Tab("📄  Key Concepts"):
                    pdf_status = gr.Markdown("Klik op '📄 Key Concepts PDF' in de sidebar.")
                    pdf_file   = gr.File(label="Download PDF", visible=False)
                with gr.Tab("📊  Tijdlijn"):
                    timeline_html = gr.HTML("<div style='color:#777;text-align:center;padding:60px 20px'>Klik op '📊 Visuele tijdlijn' in de sidebar.</div>")
                with gr.Tab("🎮  XVR Scenario"):
                    xvr_output   = gr.Textbox(label="XVR Scenario Brief", lines=20, placeholder="Klik op '🎮 XVR Scenario' in de sidebar.")
                    xvr_download = gr.File(label="Download scenario", visible=False)
    ...
    btn_pdf.click(handle_pdf,      [language], [pdf_status,pdf_file,trace_html])
